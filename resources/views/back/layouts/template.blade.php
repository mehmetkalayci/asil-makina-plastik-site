...
<nav class="sb-topnav navbar navbar-expand navbar-dark bg-dark">
    <!-- Navbar Brand-->
    <a class="navbar-brand ps-3" href="{{ route('admin.config.edit') }}">Yönetici Paneli</a>
    <!-- Sidebar Toggle-->
        ...
        <li class="nav-item dropdown">
            @auth
                <form id="logout-form" action="{{ route('admin.logout') }}" method="POST">
                    @csrf
                    <button class="btn btn-sm btn-light" type="submit">{{auth()->user()->name}} &nbsp;<i class="fas fa-right-from-bracket"></i></button>
                </form>
            @endauth
        </li>
    </ul>
